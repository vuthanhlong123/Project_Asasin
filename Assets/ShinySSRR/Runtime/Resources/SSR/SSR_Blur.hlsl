...
#define SSR_BLUR
	...
	half4 FragBlur (VaryingsCross input): SV_Target {
        ...
        #if SSR_DENOISE
            half l0 = max(0, getLuma(c0.rgb));
            half l1 = max(0, getLuma(c1.rgb));
            half l2 = max(0, getLuma(c2.rgb));
            half l3 = max(0, getLuma(c3.rgb));
            half l4 = max(0, getLuma(c4.rgb));
