                ...
                if (string.IsNullOrWhiteSpace(Get.MovingParameter) == false)
                    if (Get.Mecanim && Get.Mecanim.runtimeAnimatorController != null)
                    {
                        ...
                        catch (System.Exception exc)
                        {
                            ...
                            if (GUILayout.Button("Can't access animator properties"))
                            {
                                UnityEngine.Debug.Log("[Legs Animator] Can't access animator properties for unknown reason.\nCan't determinate if movement parameter is float or bool!\nCheck log below for details.");
                                Debug.Log(exc);
                            }
