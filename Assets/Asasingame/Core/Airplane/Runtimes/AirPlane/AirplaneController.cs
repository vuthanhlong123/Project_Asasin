...
namespace Asasingame.Core.Airplane.Runtimes
{
    ...
    public class AirplaneController : MonoBehaviour
    {
        ...
        private Vector2 currentCameraXYValue;
        ...
        [Header("Engine launch fire FX")]
        [SerializeField] private float engineLaunchFireSpeedDefault;
        [SerializeField] private float engineLaunchFireSpeedTurbo;
        [SerializeField] private float engineLaunchFireFX_speedUpdate;
        [SerializeField] private ParticleSystem[] engineLaunchFireFXs;
        ...
        [Header("MotionBlur")]
        [SerializeField] private float motionBlurDefaultValue;
        [SerializeField] private float motionBlurTurboValue;
        ...
        [SerializeField] private Vector2 shakeAmplitudeFrequencySpaceJump;
        ...
        [Header("Look")]
        [SerializeField] private Vector2 spaceJumpCameraXYAsixValue;
        [SerializeField] private float spaceJumpCameraXYAsixValueChangeSpeed;
        ...
        private void Start()
        {
            ...
            motionBlurVolume.profile.TryGet<MotionBlur>(out motionBlur);
            if(motionBlur)
            {
        ...
        private void FlyingUpdate()
        {
            ...
            if (!planeIsDead)
            {
                if (airplaneMovementState == AirplaneMovementState.Normal)
                {
                    ...
                    cameraController.EnableFreeLookControlByInput();
                }
                else if (airplaneMovementState == AirplaneMovementState.SpaceJump)
                {
                    SpaceJumpMovement();
                    cameraController.DisableFreeLookControlByInput();
                    UpdateCameraLookXYValue();
                }
            }
            else
            {
        ...
        private void Movement()
        {
            ...
            else
            {
                ...
                //Engine Fire FX
                currentEngineFireSpeed = engineLaunchFireSpeedDefault;
        ...
        private void SpaceJumpMovement()
        {
            ...
            //Shake
            currentShakeAmplitude = shakeAmplitudeFrequencySpaceJump.x;
            currentShakeFrequency = shakeAmplitudeFrequencySpaceJump.y;

            //Fov
            currentFov = turboFOV;

            //LookControl
            currentCameraXYValue = spaceJumpCameraXYAsixValue;
        }
       ...
       /* public void AddLandingRunway(Runway _landingThisRunway)
        ...
        private void UpdateEngineLaunchFireFX()
        {
            if (!planeIsDead)
            {
                foreach (ParticleSystem fx in engineLaunchFireFXs)
                {
                    var mainModule = fx.main;
                    mainModule.startLifetime = math.lerp(mainModule.startLifetime.constant, currentEngineFireSpeed, engineLaunchFireFX_speedUpdate * Time.deltaTime);
                }
            }
            else
            {
        ...
        private void UpdateMotionBlur()
        {
            if (!planeIsDead)
            {
                if(motionBlur)
                {
                    motionBlur.intensity.value = math.lerp(motionBlur.intensity.value, currentMotionBlurValue, motionBlurModifySpeed * Time.deltaTime);
        ...
        private void UpdateCameraLookXYValue()
        {
            if (!planeIsDead)
            {
                cameraController.CameraAxisControl(Vector2.Lerp(cameraController.CameraLookXYValue, currentCameraXYValue, spaceJumpCameraXYAsixValueChangeSpeed * Time.deltaTime));
            }
            else
            {
                cameraController.CameraAxisControl(Vector2.Lerp(cameraController.CameraLookXYValue, currentCameraXYValue, spaceJumpCameraXYAsixValueChangeSpeed * Time.deltaTime));
        ...
        public float InputH => inputH;
        public float InputV => inputV;
        public bool InputTurbo => inputTurbo;
        public bool InputYawLeft => inputYawLeft;
        public bool InputYawRight => inputYawRight;
        ...
        public void SpaceJumpToTargetMovement(Vector3 direction, Transform target)
        {
            spaceJumpTargetDirection = direction;
            airplaneMovementState = AirplaneMovementState.SpaceJump;
        ...
        private void HandleInputs()
        {
            ...
            moveInputSmooth = Vector2.MoveTowards(moveInputSmooth, moveInput, Time.deltaTime * smoothValue);
            inputH = moveInputSmooth.x;
